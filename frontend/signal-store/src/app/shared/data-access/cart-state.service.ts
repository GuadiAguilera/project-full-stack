...
export class CartStateService {
  ...
  private add(state: Signal<State>, productCart: ProductItemCart) {
    const isInCart = state().products.find(
      (productInCart) => productInCart.product.id === productCart.product.id
    );

    if (!isInCart) {
      return {
        products: [...state().products, { ...productCart, quantity: 1 }],
      };
    }
